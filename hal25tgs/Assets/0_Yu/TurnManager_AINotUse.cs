...
public class TurnManager_AINotUse : MonoBehaviourPun
{
    // ターンのタイプ
    ...
    static public TurnManager_AINotUse Instance { get; private set; }  // シングルトン化

    [Header("ゲーム全体タイマー")]
    [SerializeField] private Timer_AI m_GameTimer;
    // ゲーム全体タイマー終了　かつ　操作ターンイベント発生したら　ゲーム終了

    [Header("ターン計測タイマー")]
    [SerializeField] private Timer_AI m_TurnTimer;
...
// 終盤突入イベント起動関数呼出用関数
    ...
    private void AllPlayerTurnChanged()
    {
            ...
            m_CurrentTurn <= TURN_STATE.BURST_TURN)
        {
            ...
            m_GameTimer.StopTimer();    // 全体タイマー停止
        }
        else
        {
            // 現在がインターバルのとき
            ...
            m_GameTimer.ReStartTimer(); // 全体タイマー再開
        }
    }
    ...
    // 現在のターンの残り時間取得
    public Timer_AI GetTurnTimer()
    {
        return m_TurnTimer;
    }
    // 現在のゲーム全体の残り時間取得
    public Timer_AI GetGameTimer()
    {
        return m_GameTimer;
    }
