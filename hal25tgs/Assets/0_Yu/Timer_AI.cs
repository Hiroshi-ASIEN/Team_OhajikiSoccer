...
// 設定時間のタイプ
public enum TimeSetType_AI
{
   Second,  // 秒
   Minute,  // 分
}
public class Timer_AI : MonoBehaviour
{
    [Header("何のタイマーかメモ用")]
    [SerializeField] private string m_TimerName;

    [Header("設定する時間単位")]
    [SerializeField] private TimeSetType_AI m_Type = TimeSetType_AI.Second;
    ...
    private void SetTime()
    {
        switch (m_Type)
        {
            case TimeSetType_AI.Second:
                m_MaxTime *= 1.0f;
                break;

            case TimeSetType_AI.Minute:
                m_MaxTime *= 60.0f;
                break;
        }
    }
