...
public class ItemInventory : MonoBehaviourPunCallbacks
{
    ...
    private AITeamObject m_AITeam = null;
   ...
   /* [SerializeField] */private GameObject m_canvas;
    ...
    public ITEM_STATE UseItemMaster(ItemObject item)
    {
        ...
        if(TurnManager.Instance)
        {
            ...
            */
            ...
            {
                //TODO:確認
                if (m_team.GetPlayersArray()[0].GetTeamName() != ScoreManager.Instance.m_SoloTeamName)
                {
                    Debug.Log("相手チームのアイテムは使用できません。");
                    return ITEM_STATE.FAILURE;
                }
        ...
        {
            Debug.LogWarning("ターンマネージャーが存在しません");
        }
        ...
        if(m_state == ITEM_STATE.SUCCESS)
        {
            ...
            if (m_team)
            {
                ...
                Debug.LogWarning("インベントリにプレイヤーのチームが登録されていません" +
                    "（エネミーのチームならばこの表示は正常です）");
    ...
    public TeamObject GetTeamObject() { return m_team; }

    public void SetAITeam(AITeamObject _team)
    {
        m_AITeam = _team;
    }

    public AITeamObject GetAITeamObject() {  return m_AITeam; }
    ...
    //エネミー用の使用関数
    public void ConsiderUsingItemForEnemy()
    {
        //もしもアイテムをもっていたら考える
        for (int i = 0; i < m_itemList.Count; i++)
        {
            m_itemList[i].ConsiderUsingByEnemy();
        }
    }
